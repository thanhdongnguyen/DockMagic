...
let outputURL = CommandLine.arguments.count > 1
    ...
    : evidenceRoot.appendingPathComponent("interlock-crown-native-comparison.png")
...
let codexDark = try loadImage("interlock-crown-codex-detail-dark.png")
let claudeDark = try loadImage("interlock-crown-claude-detail-dark.png")
let codexLight = try loadImage("interlock-crown-codex-detail-light.png")
let codexGrayscale = try loadImage("interlock-crown-codex-detail-grayscale.png")
...
drawText(
    "DockMagic · Interlock Crown native comparison",
    x: 38,
    y: 24,
    width: 1_700,
    height: 44,
...
drawText(
    "Selected Option 3 + app-hosted SwiftUI render test · 2× backing scale",
    x: 40,
    y: 70,
    width: 1_900,
    height: 28,
...
drawText(
    "Interlocking angular ribbons · warm copper rim · graphite, ivory and cobalt enamel",
    x: 252,
    y: 172,
    width: 1_900,
    height: 32,
...
drawText(
    "The badge is the milestone symbol itself: no Codex/Claude logo, no center medallion, and no runtime identity overlay.",
    x: 252,
    y: 216,
    width: 2_150,
    height: 50,
...
drawText("CODEX · GRAYSCALE CHECK", x: 1_958, y: 316, width: 560, height: 24, font: .systemFont(ofSize: 14, weight: .bold), color: .white)
